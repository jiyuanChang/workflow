...
* **Благодаря набору механизмов жизненного цикла объектов управление памятью асинхронных программ значительно упрощается**.
	* Жизненный цикл любой задачи, созданной фреймворком, составляет от создания до завершения функции обратного вызова. Нет никакого риска утечки. Если задача создана, но пользователь не хочет ее запускать, пользователю необходимо освободить ее с помощью метода dismiss().
