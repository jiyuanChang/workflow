...
* Поддержка **asynchronous workflows**; возможность использования общих **последовательно-параллельных** структур, а также поддерживает любых структур **DAG**.
...
Механизм обратного вызова и восстановления памяти

* Все вызовы выполняются асинхронно, и почти нет ожидающей операций, которые занимают поток выполнения.
...
* Никакие интеллектуальные указатели не используются в проекте для управления памятью.
